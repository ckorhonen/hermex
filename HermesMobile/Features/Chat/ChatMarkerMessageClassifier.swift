...
enum ChatMarkerMessageClassifier {
    ...
    private static func markdownCheckboxState(in text: String) -> (state: PreservedTaskListItem.State, remainingRange: Range<String.Index>)? {
        ...
        guard text.indices.contains(markerEnd), text[markerEnd] == " " else { return nil }

        let marker = text[text.index(text.startIndex, offsetBy: 3)]
        let state: PreservedTaskListItem.State = marker == "x" || marker == "X" ? .checked : .unchecked
        return (state, markerEnd..<text.endIndex)
    }
